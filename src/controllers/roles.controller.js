    ...
    try {
        ...
        if (findAdmin.rows.length > 0) {
            ...
            if (checkPermission.rows[0].permission_to_view) {
                let s3 = dbScript(db_sql['Q14'], { var1: findAdmin.rows[0].company_id })
                let rolesList = await connection.query(s3)
                for (let data of rolesList.rows) {
                    let modulePermissions = []

                    let s4 = dbScript(db_sql['Q21'], { var1: data.id, var2: findAdmin.rows[0].company_id })
    ...
    try {
        ...
        if (findAdmin.rows.length > 0) {
            ...
            if (checkPermission.rows[0].permission_to_create) {
                ...
                let s4 = dbScript(db_sql['Q13'], { var1: roleId, var2: mysql_real_escape_string(roleName), var3: reporter, var4: findAdmin.rows[0].company_id })
                ...
                for (let moduleData of modulePermissions) {
                    ...
                    let s5 = dbScript(db_sql['Q20'], { var1: permissionId, var2: createRole.rows[0].id, var3: moduleData.moduleId, var4: moduleData.permissionToCreate, var5: moduleData.permissionToUpdate, var6: moduleData.permissionToDelete, var7: moduleData.permissionToView, var8: findAdmin.rows[0].id })
    ...
    try {
        ...
        if (findAdmin.rows.length > 0) {
            ...
            if (checkPermission.rows[0].permission_to_update) {
                ...
                await connection.query('BEGIN')
                let _dt = new Date().toISOString();
                let s4 = dbScript(db_sql['Q25'], { var1: mysql_real_escape_string(roleName), var2: reporter, var3: roleId, var4: _dt, var5: findAdmin.rows[0].company_id })

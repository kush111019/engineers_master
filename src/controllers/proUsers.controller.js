...
const {
  ...
  sourceFn,
  industryFn,
  customerFnForHubspot,
  customerFnForsalesforce,
  leadFnForsalesforce,
  leadFnForHubspot,
} = require("../utils/connectors.utils");
...
const {
  ...
  mysql_real_escape_string2,
  tranformAvailabilityArray,
  getIcalObjectInstance,
  convertToLocal,
  convertToTimezone,
  dateFormattor1,
  convertTimeToTargetedTz,
  paginatedResults,
  getParentUserList,
  getUserAndSubUser,
  calculateQuarters,
  getQuarterMonthsDates,
  calculateEOLProducts,
...
module.exports.proUserLogin = async (req, res) => {
  try {
    ...
    let s1 = dbScript(db_sql["Q329"], {
      var1: mysql_real_escape_string(emailAddress),
    });
          ...
          if (admin.rows[0].is_locked == false) {
            if (admin.rows[0].is_deactivated == false) {
              ...
              (configuration.currency = admin.rows[0].currency),
                ...
                (configuration.beforeClosingDays = admin.rows[0]
                  .before_closing_days
                  ? admin.rows[0].before_closing_days
                  : ""),
                (configuration.afterClosingDays = admin.rows[0]
                  .after_closing_days
                  ? admin.rows[0].after_closing_days
                  : "");
              // fetching imap_credentail for the admin
              let s2 = dbScript(db_sql["Q125"], {
                var1: admin.rows[0].id,
                var2: admin.rows[0].company_id,
              });
              ...
              for (let data of moduleId) {
                ...
                let s3 = dbScript(db_sql["Q58"], {
                  var1: data,
                  var2: admin.rows[0].role_id,
                });
              ...
              res.send({
                ...
                data: {
                  ...
                  expiryDate:
                    admin.rows[0].role_name == "Admin"
                      ? admin.rows[0].expiry_date
                      : "",
...
module.exports.usersList = async (req, res) => {
  try {
    ...
    if (findAdmin.rowCount > 0 && isProUser) {
      ...
      let s4 = dbScript(db_sql["Q314"], {
        var1: findAdmin.rows[0].company_id,
        var2: false,
      });
...
module.exports.connectorsList = async (req, res) => {
  try {
    ...
    if (findUser.rowCount > 0 && isProUser) {
      ...
      let s2 = dbScript(db_sql["Q317"], {
        var1: userId,
        var2: findUser.rows[0].company_id,
      });
...
module.exports.authUrl = async (req, res) => {
  try {
    ...
    if (provider.toLowerCase() == "hubspot") {
      const scope = [
        "content tickets e-commerce crm.objects.contacts.read crm.objects.custom.read crm.objects.custom.write media_bridge.read crm.objects.goals.read",
      ];
    ...
    if (findUser.rowCount > 0 && isProUser) {
      if (provider.toLowerCase() == "linkedin") {
        ...
        LinkedIn.auth.getAccessToken(code, state, async (err, results) => {
          ...
          if (getConnectors.rowCount == 0) {
            ...
            let s4 = dbScript(db_sql["Q319"], {
              var1: "linked_in_token",
              var2: accessToken,
              var3: "linked_in_status",
              var4: true,
              var5: _dt,
              var6: userId,
              var7: findUser.rows[0].company_id,
            });
      ...
      if (provider.toLowerCase() == "hubspot") {
        ...
        if (getConnectors.rowCount == 0) {
          ...
          let s3 = dbScript(db_sql["Q323"], {
            var1: userId,
            var2: findUser.rows[0].company_id,
            var3: token.accessToken,
            var4: true,
            var5: token.refreshToken,
            var6: expiry,
          });
          let storeAccessToken = await connection.query(s3);
          if (storeAccessToken.rowCount > 0) {
          ...
          let _dt = new Date().toISOString();
          let s4 = dbScript(db_sql["Q320"], {
            var1: token.accessToken,
            var2: true,
            var3: token.refreshToken,
            var4: expiry,
            var5: userId,
            var6: findUser.rows[0].company_id,
          });
          let storeAccessToken = await connection.query(s4);
          if (storeAccessToken.rowCount > 0) {
            await connection.query("COMMIT");
      ...
      if (provider.toLowerCase() == "salesforce") {
        ...
        axios
          ...
          .then(async (response) => {
            ...
            if (getConnectors.rowCount == 0) {
              //sotring the access token if not already stored
              let s3 = dbScript(db_sql["Q321"], {
                var1: userId,
                var2: findUser.rows[0].company_id,
                var3: response.data.access_token,
                var4: true,
                var5: response.data.refresh_token,
                var6: expirationTime,
              });
              let storeAccessToken = await connection.query(s3);
              if (storeAccessToken.rowCount > 0) {
              ...
              //updating the access token if already stored
              let _dt = new Date().toISOString();
              let s4 = dbScript(db_sql["Q325"], {
                var1: response.data.access_token,
                var2: true,
                var3: response.data.refresh_token,
                var4: expirationTime,
                var5: userId,
                var6: findUser.rows[0].company_id,
              });
        ...
        try {
          ...
          if (expiryDate < curDate) {
            ...
            axios
              ...
              .then(async (res) => {
                ...
                let s4 = dbScript(db_sql["Q325"], {
                  var1: res.data.access_token,
                  var2: true,
                  var3: accessData.salesforce_refresh_token,
                  var4: expirationTime,
                  var5: accessData.user_id,
                  var6: accessData.company_id,
                });
                let storeAccessToken = await connection.query(s4);
              })
              .catch((err) => {
          ...
          axios
            ...
            .then((response) => {
              ...
              const apiUrl =
                `${response.data.urls.custom_domain}` +
                `${process.env.SALESFORCE_API_VERSION}`;
              const query =
                "SELECT Id,Name,Title,Company,Street,City,State,Country,Address,Phone,Email,Website,Description,LeadSource,Industry,LastModifiedDate,createdDate FROM Lead";
              axios({
                method: "get",
                ...
                .then(async (response) => {
                  if (response.data.records.length > 0) {
                    ...
                    let s1 = dbScript(db_sql["Q308"], {
                      var1: accessData.company_id,
                    });
                    let findSyncLead = await connection.query(s1);
                    //Initial insertion
                    if (findSyncLead.rowCount == 0) {
                      for (let data of response.data.records) {
                        let titleId = await titleFn(
                          data.Title,
                          accessData.company_id
                        );

                        let sourceId = await sourceFn(
                          data.LeadSource,
                          accessData.company_id
                        );

                        let industryId = await industryFn(
                          data.Industry,
                          accessData.company_id
                        );

                        let customerId = await customerFnForsalesforce(
                          data,
                          accessData,
                          industryId
                        );

                        let leads = await leadFnForsalesforce(
                          titleId,
                          sourceId,
                          customerId,
                          data,
                      ...
                      for (let data of response.data.records) {
                        if (
                          new Date(accessData.salesforce_last_sync) <
                          ...
                          let s10 = dbScript(db_sql["Q322"], {
                            var1: data.Id,
                            var2: accessData.company_id,
                          });
                    ...
                    let _dt = new Date().toISOString();
                    let s12 = dbScript(db_sql["Q278"], {
                      var1: _dt,
                      var2: accessData.company_id,
                    });
                    let updateStatusInCompany = await connection.query(s12);

                    let s11 = dbScript(db_sql["Q324"], {
                      var0: "salesforce_last_sync",
                      var1: _dt,
                      var2: _dt,
                      var3: accessData.user_id,
                      var4: accessData.company_id,
                    });
          ...
          if (leadsData.length > 0) {
            ...
            if (findSyncLead.rowCount == 0) {
              ...
              for (let data of leadsData) {
                ...
                let customerId = await customerFnForHubspot(
                  data,
                  accessData,
                  industryId
                );
                ...
                let leads = await leadFnForHubspot(
                  leadName,
                  titleId,
                  sourceId,
                  customerId,
              ...
              for (let data of leadsData) {
                if (
                  new Date(accessData.hubspot_last_sync) <
...
module.exports.leadReSync = async (req, res) => {
  ...
  if (findUser.rowCount > 0 && isProUser) {
    ...
    for (let accessData of getConnectors.rows) {
      if (
        provider.toLowerCase() == "salesforce" &&
        accessData.salesforce_status
      ) {
        await connection.query("BEGIN");
          ...
          if (salesData.sales_users) {
            salesData.sales_users.map((value) => {
              if (value.user_type == process.env.CAPTAIN) {
                ...
                value.user_commission_amount = salesData.booking_commission
                  ? (Number(value.percentage) / 100) *
                    salesData.booking_commission
                  : 0;
              } else {
...
module.exports.salesDetails = async (req, res) => {
  try {
    ...
    if (
      (isProUser && checkPermission.rows[0].permission_to_view_global) ||
      checkPermission.rows[0].permission_to_view_own
    ) {
      let s3 = dbScript(db_sql["Q421"], {
        var1: checkPermission.rows[0].company_id,
        var2: salesId,
      });
...
module.exports.getUpperLevelUserList = async (req, res) => {
  try {
    ...
    if (roleData.rows[0].reporter) {
      let parentList = await getParentUserList(
        roleData.rows[0],
      ...
      res.json({
        ...
        success: false,
        message:
          "You are the admin role holder you don't need to request for that ",
      });
    }
  } catch (error) {
    res.json({
...
module.exports.recognizationDetailsPro = async (req, res) => {
  try {
    ...
    if (findUser.rowCount > 0 && isProUser) {
      ...
      let s3 = dbScript(db_sql["Q249"], {
        var1: findUser.rows[0].company_id,
        var2: salesId,
      });
      let salesList = await connection.query(s3);
      let salesObj = {};
...
module.exports.createProEmailTemplate = async (req, res) => {
  try {
    ...
    if (findUser.rowCount > 0 && isProUser) {
      ...
      let s2 = dbScript(db_sql["Q330"], {
        var1: userId,
        var2: findUser.rows[0].company_id,
        var3: mysql_real_escape_string2(emailTemplate),
        var4: templateName,
        var5: mysql_real_escape_string2(jsonTemplate),
      });
...
module.exports.updateEmailTemplate = async (req, res) => {
  try {
    ...
    if (findUser.rowCount > 0 && isProUser) {
      ...
      let _dt = new Date().toISOString();
      let s2 = dbScript(db_sql["Q332"], {
        var1: templateId,
        var2: _dt,
        var3: templateName,
        var4: mysql_real_escape_string2(emailTemplate),
        var5: mysql_real_escape_string2(jsonTemplate),
      });
...
module.exports.sendEmailToLead = async (req, res) => {
  try {
    ...
    if (findAdmin.rowCount > 0 && isProUser) {
      let s2 = dbScript(db_sql["Q125"], {
        var1: findAdmin.rows[0].id,
        var2: findAdmin.rows[0].company_id,
      });
      ...
      if (findCreds.rowCount > 0) {
        ...
        res.json({
          ...
          success: false,
          message:
            "SMTP credentials not available. Please add SMTP credentials first.",
        });
      }
    } else {
      res.status(403).json({
    ...
    if (findAdmin.rows.length > 0 && isProUser) {
      ...
      promise
        .then(async (data) => {
          ...
          let s4 = dbScript(db_sql["Q125"], {
            var1: findAdmin.rows[0].id,
            var2: findAdmin.rows[0].company_id,
          });
          let findSmtpcreds = await connection.query(s4);
          if (findSmtpcreds.rowCount == 0) {
            let s3 = dbScript(db_sql["Q341"], {
              var1: email,
              var2: encryptedAppPassword,
              var3: findAdmin.rows[0].id,
              var4: smtpHost,
              var5: smtpPort,
              var6: findAdmin.rows[0].company_id,
            });
            ...
            let s5 = dbScript(db_sql["Q361"], {
              var1: email,
              var2: encryptedAppPassword,
              var3: smtpHost,
              var4: smtpPort,
              var5: findSmtpcreds.rows[0].id,
              var6: _dt,
            });
  ...
  try {
    ...
    if (findAdmin.rows.length > 0 && isProUser) {
      let s2 = dbScript(db_sql["Q342"], {
        ...
        var2: timezone,
        var3: userId,
        var4: findAdmin.rows[0].company_id,
      });
        ...
        if (ts.checked) {
          for (let subTs of ts.timeSlots) {
            ...
            let s3 = dbScript(db_sql["Q343"], {
              ...
              var5: findAdmin.rows[0].company_id,
              var6: ts.checked,
              var7: userId,
            });
  ...
  try {
    ...
    if (showEventDetails.rowCount > 0) {
      if (showEventDetails.rows[0].availability_time_slots.length > 0) {
        let finalArray = await tranformAvailabilityArray(
          showEventDetails.rows[0].availability_time_slots
        );
        for (let item of finalArray[0].time_slots) {
          for (let slot of item.time_slot) {
            let { localStart, localEnd } = await convertToTimezone(
...
module.exports.scheduleEvent = async (req, res) => {
  try {
    ...
    const result = await convertTimeToTargetedTz(
      startTime,
      endTime,
      leadTimezone,
      date,
    ...
    let calObjCreator = await getIcalObjectInstance(
      result.startTargetedTimezoneStringIso,
      result.endTargetedTimezoneStringIso,
      eventName,
      description,
      location,
      meetLink,
    ...
    // for lead ---------------------------------------
    let leadDates = await dateFormattor1(
      date,
      startTime,
      endTime,
      leadTimezone
    );
    let calObjLead = await getIcalObjectInstance(
      leadDates.startDate,
      leadDates.endDate,
      eventName,
      description,
    ...
    let s1 = dbScript(db_sql["Q349"], {
      var1: eventId,
      var2: result.startTargetedTimezoneStringIso,
      var3: result.startTargetedTimezoneStringIso,
      var4: result.endTargetedTimezoneStringIso,
      var5: mysql_real_escape_string(leadName),
      var6: leadEmail,
      var7: mysql_real_escape_string(description),
      var8: userId,
      var9: companyId,
      var10: creatorTimezone,
    });
    ...
    if (captainId) {
      ...
      if (findAdmin.rowCount > 0 && isProUser) {
        ...
        if (salesIds.rowCount > 0) {
          ...
          let s3 = dbScript(db_sql["Q364"], {
            var1: captainId,
            var2: salesIdArr.join(","),
          });
          let salesDetails = await connection.query(s3);

          if (salesDetails.rowCount > 0) {
            let s4 = dbScript(db_sql["Q365"], {
              var1: captainId,
              var2: salesIdArr.join(","),
            });
            ...
            let notesCountArr = updatedSalesDetails.map((detail) =>
              Number(detail.notes_count || 0)
            );
      ...
      if (commissionData.rowCount > 0) {
        ...
        for (let row of commissionData.rows) {
          ...
          if (row.sales_type === "Perpetual") {
            data.totalPerpetualCommissionEarned += Number(
              row.commission_amount
            );
          } else if (row.sales_type === "Subscription") {
            data.totalSubscriptionCommissionEarned += Number(
              row.commission_amount
            );
          }

          data.totalCommission =
            data.totalPerpetualCommissionEarned +
            data.totalSubscriptionCommissionEarned;
        }
        if (data) {
          res.json({
            status: 200,
    ...
    if (findAdmin.rowCount > 0 && isProUser) {
      ...
      if (includeStatus !== true) {
        ...
        if (findSalesActivities.rowCount > 0) {
          const { total_sales_activities, total_deals_created } =
            findSalesActivities.rows[0];
          salesActivities.total_sales_activities = Number(
            total_sales_activities
          );
          salesActivities.total_deals_created = Number(total_deals_created);
          salesActivities.activity_per_deal =
            salesActivities.total_deals_created !== 0
              ? total_sales_activities / total_deals_created
              : 0;
        } else {
        ...
        if (allSalesIds.rowCount > 0) {
          ...
          if (findTotalAndWonDealCount.rowCount > 0) {
            const { total_sales_count, closed_sales_count } =
              findTotalAndWonDealCount.rows[0];
            totalAndWonDealCount.total_sales_count = Number(total_sales_count);
            totalAndWonDealCount.closed_sales_count =
              Number(closed_sales_count);
            totalAndWonDealCount.winPercentage =
              total_sales_count > 0
                ? (closed_sales_count / total_sales_count) * 100
                : 0;
          } else {
          ...
          const yearlyStartFormattedDate = new Date(currentYear, 0, 1)
            .toISOString()
            .substring(0, 10); // Month is 0-based, so 0 is January

          // Create the end date of the year
          const yearlyEndFormattedDate = new Date(currentYear, 11, 31)
            .toISOString()
            .substring(0, 10);
          ...
          if (findTotalSupport.rowCount > 0) {
            for (const sale of findTotalSupport.rows) {
              ...
              const salesDeals = isHighRisk
                ? high_risk_sales_deals
                : low_risk_sales_deals;
              salesDeals.push({
                salesId: sale.sales_id,
                salesName: sale.customer_name,
                amount: sale.target_amount,
              });
            }
            totalHighRiskAmount = high_risk_sales_deals.reduce(
              (total, sale) => total + parseInt(sale.amount),
              0
            );
            totalLowRiskAmount = low_risk_sales_deals.reduce(
              (total, sale) => total + parseInt(sale.amount),
              0
            );
            ...
            risk_sales_deals.total_sales_deals_amount =
              totalHighRiskAmount + totalLowRiskAmount;
          } else {
          ...
          if (findMissingRecognized.rowCount > 0) {
            findMissingRecognized.rows.forEach((item) => {
              if (item.recognized_amount === "0") {
                ...
                low_risk_missing_rr[item.sales_id].recognized_amount +=
                  parseFloat(item.recognized_amount);
              }
            });
          ...
          if (findDateSlippage.rowCount > 0) {
            ...
            for (const salesId in salesBySalesId) {
              ...
              if (salesInfo.targetClosingDates.size > 1) {
                high_risk_sales.push({
                  sales_id: salesId,
                  customer_name: salesInfo.customer_name,
                  missing_amount: salesInfo.amount,
                });
              } else {
                low_risk_sales.push({
                  sales_id: salesId,
            ...
            totalHighRiskSlippageAmount = high_risk_sales.reduce(
              (total, sale) => total + parseInt(sale.missing_amount),
              0
            );
            totalLowRiskSlippageAmount = low_risk_sales.reduce(
              (total, sale) => total + parseInt(sale.missing_amount),
              0
            );
            ...
            closingDateSlippage.total_high_risk_slippage_amount =
              totalHighRiskSlippageAmount;
            closingDateSlippage.total_low_risk_slippage_amount =
              totalLowRiskSlippageAmount;
            closingDateSlippage.all_total_slippage_amount = Number(
              totalHighRiskSlippageAmount + totalLowRiskSlippageAmount
            );
          } else {
            closingDateSlippage.high_risk_sales = high_risk_sales;
            closingDateSlippage.low_risk_sales = low_risk_sales;
          ...
          let totalHighRiskEolMissingAmount = eolSales.highRiskEolSale.reduce(
            (total, sale) => total + parseInt(sale.target_amount),
            0
          );
          let totalLowRiskEolMissingAmount = eolSales.lowRiskEolSale.reduce(
            (total, sale) => total + parseInt(sale.target_amount),
            0
          );
          eolSales.total_high_risk_eol_missing_amount =
            totalHighRiskEolMissingAmount;
          eolSales.total_low_risk_eol_missing_amount =
            totalLowRiskEolMissingAmount;
          eolSales.all_total_eol_missing_amount = Number(
          ...
          if (findForecastAmount.rowCount > 0) {
            ...
            revenueGap =
              totalForecaseAmount - Number(totalYearlyRecognizedRevenue);
          } else {
        ...
        if (allSalesIds.rowCount > 0) {
          ...
          if (findTotalSupport.rowCount > 0) {
            ...
            risk_sales_deals.high_risk_sales_deals =
              high_risk_sales_deals.filter((hr) => hr?.amount?.toString() != 0);
            risk_sales_deals.low_risk_sales_deals = low_risk_sales_deals.filter(
              (hr) => hr?.amount?.toString() != 0
            );
            risk_sales_deals.total_high_risk_amount = totalHighRiskAmount;
            risk_sales_deals.total_low_risk_amount = totalLowRiskAmount;
            risk_sales_deals.total_sales_deals_amount =
          ...
          if (findMissingRecognized.rowCount > 0) {
            ...
            Object.values(low_risk_missing_rr).forEach((item) => {
              const amountDifference =
                item.target_amount - item.recognized_amount;
              item.amount = amountDifference;
            });
            ...
            missingRR.high_risk_missing_rr = high_risk_missing_rr.filter(
              (hr) => hr?.amount?.toString() != 0
            );
            missingRR.low_risk_missing_rr = Object.values(
              low_risk_missing_rr
            ).filter((hr) => hr?.amount?.toString() != 0);
          } else {
            missingRR.high_risk_missing_rr = [];
            missingRR.low_risk_missing_rr = [];
          }
          ...
          if (findDateSlippage.rowCount > 0) {
            ...
            closingDateSlippage.high_risk_sales = high_risk_sales.filter(
              (hr) => hr?.amount?.toString() != 0
            );
            closingDateSlippage.low_risk_sales = low_risk_sales.filter(
              (hr) => hr?.amount?.toString() != 0
            );
            closingDateSlippage.total_high_risk_slippage_amount =
              totalHighRiskSlippageAmount;
...
module.exports.getAllApiDeatilsRelatedSales = async (req, res) => {
  try {
    ...
    if (checkPermissionForProduct.rows[0].permission_to_view_global) {
      ...
      let s2 = dbScript(db_sql["Q84"], {
        var1: checkPermissionForProduct.rows[0].company_id,
      });
      ...
      let roleUsers = await getUserAndSubUser(
        checkPermissionForProduct.rows[0]
      );
    ...
    if (checkPermissionForCustomer.rows[0].permission_to_view_global) {
      ...
      let s3 = dbScript(db_sql["Q39"], {
        var1: checkPermissionForCustomer.rows[0].company_id,
      });
      ...
      let roleUsers = await getUserAndSubUser(
        checkPermissionForCustomer.rows[0]
      );

      let s2 = dbScript(db_sql["Q271"], {
        var1: roleUsers.join(","),
        var2: false,
      });
    ...
    if (checkPermissionForslab.rows[0].permission_to_view_global) {
      ...
      let s5 = dbScript(db_sql["Q17"], {
        var1: checkPermissionForslab.rows[0].company_id,
      });
      let slabList = await connection.query(s5);
      if (slabList.rowCount > 0) {
        const unique = [
          ...new Map(
            slabList.rows.map((item) => [item["slab_id"], item])
          ).values(),
        ];
    ...
    if (checkPermissionForCommission.rows[0].permission_to_view_global) {
      ...
      let s6 = dbScript(db_sql["Q50"], {
        var1: checkPermissionForCommission.rows[0].company_id,
      });
      ...
      let roleUsers = await getUserAndSubUser(
        checkPermissionForCommission.rows[0]
      );

      let s5 = dbScript(db_sql["Q50"], {
        var1: checkPermissionForCommission.rows[0].company_id,
      });
    ...
    if (checkPermissionForusers.rows[0].permission_to_view_global) {
      ...
      let s4 = dbScript(db_sql["Q15"], {
        var1: checkPermissionForusers.rows[0].company_id,
      });
...
const generateDateIntervals = (startDate, endDate, slot) => {
  const intervals = [];
